...
\section*{[P15]}
\subsection*{E62}
(200) holds since the algorithm is equivalent to CGD if we set
\begin{equation*}
    \begin{split}
        x^k &:= h_i^k, \\
        g^k &:= \mathcal{C}_i^k(\nabla f_i(x^k) - h_i^k).
...
\end{equation*}
can be interpreted as a \textit{descent} rule instead of \textit{ascent}, and minimize instead of maximize.
